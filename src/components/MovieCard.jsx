  ...
  return (
    <a href={`/movie/${movie.id}`} className="group relative block overflow-hidden rounded-2xl bg-slate-800 transition-all hover:-translate-y-2 hover:shadow-[0_10px_40px_rgba(99,102,241,0.25)] hover:ring-1 hover:ring-indigo-500/50">
      ...
      <div className="absolute bottom-0 left-0 p-5 w-full transform transition-transform duration-300">
        ...
        {movie.release_date && (
          <p className="text-sm font-medium text-slate-400 mt-2">{new Date(movie.release_date).getFullYear()}</p>
        )}
        {movie.rating && (
          <div className="mt-3 flex items-center gap-2">
            <span className="bg-gradient-to-r from-indigo-500/30 to-purple-500/30 text-indigo-200 text-xs font-bold px-3 py-1.5 rounded-lg border border-indigo-500/30 shadow-[0_0_10px_rgba(99,102,241,0.2)]">
              ★ {movie.rating}/10
            </span>
          </div>
        )}
